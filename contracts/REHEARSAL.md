...
7. Exercise the full application path: existing V1 claim → fresh X OAuth → SIWE proof → binding → frozen metadata → durable authorization → linked-wallet mint → unfinalized → finalized → Gallery → transfer → holder update. Confirm claimant and initial minter remain unchanged after transfer.
8. Exercise wallet rejection, stale X auth, invalid/replayed SIWE, metadata and signer outages, transaction revert/replacement, browser loss after submission, indexer restart, shallow reorg, RPC disagreement, exact authorization revocation, epoch revocation, pause, delayed-admin-only unpause, and transfer while minting is paused.
