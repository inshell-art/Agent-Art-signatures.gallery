...
## Active refactor: preview / mint & reveal — 2026-09-15

Branch: `codex/open-grok-mint`, starting from `checkpoint-pre-refactor-2026-09-13`. The **default app is now open mint**, not the X-claim flow described in the historical sections below. Read [open-mint architecture](docs/open-mint.md) and the new README introduction first.

- User validation (2026-09-15): the user confirmed a successful injected-wallet local mint after resetting Rabby's local nonce/history. The separately approved queued nonce412/413 transactions were removed only after exact pool checks; Anvil state and existing artifacts were preserved. This validates the local fixture mint path, not a paid Grok assessment or public deployment.
- The working preview/mint and wallet recovery flow was committed as `4ee47ac` before adopting the new renderer. The requested renderer upgrade is separate from that checkpoint; no push was requested.

- `src/main.ts` starts `src/openMint/main.ts`; the old entrypoint is `src/legacyMain.ts`. Existing claims, OAuth credentials, PostgreSQL and `.local/rehearsal/` are preserved, not migrated into the new identity model.
- Consumer Grok asks for a handle, assesses MBTI in the user's chat, and returns an editable `/s/<exactCaseHandle>/<MBTI>` preview. GET renders only that variant, with no trusted assessment or paid API call. Its bridge carries only the handle to `/mint?handle=…`.
- Anyone can mint an available handle once, with case-insensitive uniqueness. `/mint` requires a fresh signed-wallet session before explicit **Mint & reveal** can initiate independent Grok + native X Search. Backend result fields are not accepted from the client. Private `/mint/<code>` hides the final artwork until verified chain confirmation, then redirects to the minted permalink. Home and `/me` list minted tokens only. This is UI reveal, not cryptographic secrecy; the existing contract still commits the artifact before minting.
- Canonical assessments are reused across cancelled transactions, expired requests and later sessions. Durable per-handle provider admission prevents an interrupted/stale-pending request from triggering uncounted automatic retries. Failed or uncertain uncached calls need operator reconciliation. Wallet proof is a cost gate, not complete abuse protection.
- Local wallet safety (2026-09-15): injected-wallet flows verify the backend's canonical block fingerprint, not only shared Anvil chain ID31337; mint submission uses an explicit fresh nonce and a wallet-RPC call simulation. The approved nonce410 transaction and a later, separately approved nonce411 transaction on18548 were each removed only after exact singleton-pool checks, without resetting chain state/history; the sender's next nonce remained1. An explicit dapp nonce does not guarantee the wallet's signed nonce, and equal latest/pending counts do not rule out a queued nonce gap. After app restart, reconnect at `/mint?handle=<handle>` for a fresh session/request; the saved assessment is reused. See `docs/open-mint.md` for guards and recovery details.
- Submission recovery now verifies the broadcast nonce after sending and on polls, reports nonce gaps, distinguishes delayed/missing transactions after30s, and retains the hash/duplicate guard. Read-only inspection and report/status calls are bounded to8s; late inspections cannot release the guard. Reverted receipts require matching transaction and canonical block evidence. Rabby's custom-network history can override the dapp nonce; manually set **Advanced Settings → Nonce** to the fresh node value for a retry, without clearing account-wide wallet history. No actual Rabby mint was submitted during this repair.
- Active previews and new assessments use native handle + MBTI **renderer v2.0.0**, pinned to prototype commit `4bcc513b53edac6961e385604cd9fcc3ac913cbb`. See `reference/algorithm-v2.0.0/README.md`. Its five-file source/port/oracle pin checks384 Python-generated SVG goldens; the unchanged v1 lock still checks six files/17 goldens. Legacy seed-mapped assessments/artifacts remain readable and immutable, including unminted cached results. Native v2 assessments have no synthetic seed/mapping and use a separate digest domain. Requested spelling remains exact-case; only research/uniqueness use lowercase. Neither this upgrade nor later case variants open another mint slot. Do not reset/reseed the chain or rewrite old assets to make them look like v2.
- Renderer upgrade verification: **2,634 tests /100 files**,65 contract tests, typecheck/build and both renderer pins pass. Coverage94.37% statements/lines,88.95% branches,97.59% functions; thresholds unchanged. An independent2,096-case Python differential check also passed. Live3003 previews match all16 native MBTIs, with renderer-versioned image URLs to avoid cached v1 output. Browser evidence:16 loaded square images, four columns at742px and two at390px, no horizontal overflow; viewport override restored. Existing3 minted pages/assets and all40 record/four assessment file hashes are unchanged after a graceful restart. A separate fixture app3015/Anvil18559 completed a v2 mint/reveal (`check_221c68`) and verified SVG/PNG hashes; the user's3003/18548 chain was not used for that test. No paid Grok calls. Current development app remains3003 with its saved state; app restart clears in-memory wallet sessions.
- Run `npm run dev:open` for real Grok + isolated Anvil, or `npm run dev:fixture` for explicitly simulated assessments + real local test transactions. Missing `XAI_API_KEY` fails closed. The existing X OAuth key cannot substitute for xAI credentials. No live xAI assessment has been validated in this task because the key was absent.
- Existing inspection apps still occupy ports3000 and3002, with Anvil18547 occupied; they were not stopped or reset for the 2026-09-15 refactor. Latest inspection app is `http://127.0.0.1:3003`, Anvil18548, data `.local/open-mint/preview-reveal-20260915/`, explicit fixture mode with no paid API calls. It was gracefully restarted after the final browser-event fix, preserving its two test tokens. Execution approval suffered intermittent capacity errors; it eventually allowed startup and restart. Do not remove writer locks or reset data to work around an occupied port.
- Local metadata uses verified content-addressed HTTP URLs, not public IPFS publication. `NODE_ENV=production` refuses startup; public deployment and production adapters/security review remain explicitly out of scope.
- Current verification: **2,184 tests across98 files pass**, including121 client lifecycle tests and the real-loopback HTTP transport (`OPEN_MINT_TEST_HTTP=1 npm run test:coverage`); coverage94.23% statements/lines,88.82% branches,97.5% functions. Typecheck/build and renderer lock pass; contracts were unchanged by the wallet fix (previous65 Solidity tests passed). Thresholds unchanged. Initial default-sandbox EPERM failures were resolved by rerunning with authorized socket/process access. HTTP tests also support actual request-handler execution in memory when sockets are unavailable.
- Wallet-recovery visual QA: real page/client/CSS with an isolated simulated provider passed390px dark nonce-gap and stalled-RPC cases plus1440px light missing-transaction feedback. No overflow, signing or transaction sends. A never-settling wallet read timed out while canonical server polling continued. Updated dev client `/assets/open-mint-af13d531e7d6556c.js` was verified on3003 after graceful restart;18548 retained an empty pool and next nonce1. These UI checks did not access or approve the user's wallet.
- Actual isolated-chain/API rehearsal passed preview navigation, wallet-before-assessment, forged MBTI rejection, private request access, no early result/permalink, explicit consent, real test mint, duplicate rejection, collection visibility and immutable assets. Browser flow from a verified wallet completed **Mint & reveal → preparing → confirmed permalink**, with no intermediate artwork review. Test tokens: `check_46a5d5` and `Reveal_Study`; these are fixtures, not real Grok assessments.
- Visual verification:1440px light preview has one672px artwork and handle-only bridge;390px dark mint form is wallet-gated with no images;390px dark revealed page after restart has one350px artwork, no mint CTA, no horizontal overflow. All observed resources200. Browser testing caught the native `event.currentTarget`-after-await bug in DEV wallet connection; fixed and tested with a harness that clears currentTarget after dispatch. The final repeated click check after restart was blocked by browser approval-service capacity, though the updated client asset and persisted minted artwork were confirmed by the already-running post-restart visual check. Do not claim a fresh injected-wallet/live-Grok test.
- Run `npm run open:rehearsal -- --execute-local-test-transactions` against the explicit fixture app on3002 to repeat the local mutation test. This creates new **test** artwork/token; do not target legacy rehearsal or public networks. The test script refuses a non-fixture/nonlocal app.

Everything below is historical checkpoint context, not the current product specification.
