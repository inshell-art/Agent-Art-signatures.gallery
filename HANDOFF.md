...
- The renderer lock is unchanged: six source files and 17 full SVG goldens verified. New `mbti-seed-v1` maps the 16 MBTIs into existing integer seeds. Requested spelling is preserved after removing the leading `@`; lowercase is only the research lookup and identity key. The first saved artwork freezes its exact rendering spelling, so later case variants reuse it rather than creating another artwork or mint slot. Existing artifacts are untouched; records without a separate rendering spelling keep their original lowercase render input.
