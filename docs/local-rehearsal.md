...
1. Open <http://127.0.0.1:3000/s/alice/73> and select **Sign in with X and claim this signature**. If this exact signature was already claimed, choose a different valid `gr0k` value for a fresh work.
...
4. Review the exact artwork, recipient, local chain, project fee **None**, wallet-estimated network gas, and publication disclosure. **Change recipient** before issuance returns to wallet selection and requires a new proof, not another X trip. Acknowledge the disclosure, then authorize and explicitly confirm the local transaction. The signed transaction is submitted to Anvil; the page waits for observed contract evidence before showing Minted. Prepared/live/unresolved authorizations block recipient changes. Cancel does not erase issued authority; retry resumes the same exact work and recipient.
