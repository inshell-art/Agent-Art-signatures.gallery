...
The next chain-to-gallery integration needs an explicit finality policy. Proposed to the user: show inclusion as pending, then reveal/list only once two independent RPC sources agree on finalized confirmation. This checkpoint does not assume approval, select a confirmation count, or conflate current-head eligibility with finality. This is a substantial UX choice: [Ethereum's current finality explanation](https://ethereum.org/roadmap/single-slot-finality) describes approximately 15 minutes, not an instant reveal. Actual Sepolia timing must be observed; no elapsed timer substitutes for chain evidence. A faster provisional reveal would require a separately agreed pending/reorg presentation policy.
...
The [first hosted run](https://github.com/inshell-art/Agent-Art-signatures.gallery/actions/runs/35505306730) passed 4,550 tests and failed one pre-existing PostgreSQL timeout-phase test: on the slower runner its 30 ms deadline expired before X dispatch, not while its receipt was queued. The queued-receipt test now waits for that real SQL phase before advancing controlled timer/monotonic time; the adjacent hung-refresh test uses the same phase control. Production behavior and all durable fence assertions are unchanged. The replacement run must pass separately; a local pass is not a hosted result.
