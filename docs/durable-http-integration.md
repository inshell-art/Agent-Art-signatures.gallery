...
These are private integration endpoints, **not yet a complete browser-service replacement**: wallet chain-context/nonce/simulation, canonical mint-status/transaction-hint handling, confirmed reveal/pages, galleries and public assets remain unwired. In particular, the current browser client expects a validated network/nonce context which this isolated authorization response does not yet supply. Do not redirect the running UI to this listener.
