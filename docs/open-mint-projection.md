...
A safety halt already saved before process restart remains a terminal `safety-halted` sync result without an RPC request. Lost/closed database ownership returns terminal `writer-unavailable`; it is not relabeled as a temporary chain outage or automatically retried by the poller. Both cases require an explicit operator recovery/restart path.
