...
V1 accepts a normalized X handle and a fixed-point `gr0k` value, renders a side-effect-free preview, authenticates the matching account through X OAuth, and saves the exact artwork after explicit claim consent. V2 adds an opt-in Ethereum mint path without changing the artwork.
...
Open http://localhost:3000. Development fixtures are enabled by default. Claimed contains 14 stored rehearsal claims; Minted contains the 12 explicitly simulated finalized works across four desktop rows. Both tabs use the same [fixture library](src/v2/galleryFixtures.ts): eleven recognizable public X handles from digital art and technology, plus Alice's three interactive rehearsal claims. Only the public handle strings are real; all seeded identities, `gr0k` values, claims, wallets, mints and transfers are fictional, with no participation or endorsement implied. Each library entry records a primary source for its handle, and home cards are labeled “Fictional demo.” Some show a simulated transfer so claimant, initial recipient, and current holder are visibly distinct. These samples use stored renderer output and working detail pages, not placeholder images. They are seeded only by this in-memory entrypoint; the durable PostgreSQL + Anvil rehearsal retains its separate chain-backed seed. Both tabs use centered, same-color labels with a background box on the selected tab, server-rendered links and stable keyset pagination; switching tabs starts at that gallery's first page and works without JavaScript.
...
3. Open `/s/alice/0.371924` → **Claim with X**. Authentication saves the public claim and returns directly to its detail page. A failed save offers **Retry claim**. Ordinary account sign-in creates no claim.
...
The bundled renderer, wallet, signer, chain, IPFS, and finality fixtures exist only so the product can be inspected locally. They are not production provenance. Production startup still fails closed. Mainnet deployment is explicitly forbidden without a separate approval.
