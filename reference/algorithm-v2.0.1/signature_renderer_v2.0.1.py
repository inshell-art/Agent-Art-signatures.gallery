#!/usr/bin/env python3
"""Signature Renderer v2.0.1: deterministic X-handle + MBTI -> SVG."""
...
REFERENCE_CURVE_SPAN = SHARED_SETTINGS["curve_span_px"]
REFERENCE_SEGMENT_WIDTH = REFERENCE_CURVE_SPAN / (SIGNATURE_CHARACTER_LIMIT - 1)
REFERENCE_HORIZONTAL_MARGIN = (CANONICAL_WIDTH - REFERENCE_CURVE_SPAN) / 2
...
def reusable_text_curve_layout(character_count: int) -> dict[str, float | int | bool]:
    """Return the required canvas expansion policy for non-Signature text.

    The Signature entry point remains limited to X handles of 1–15 characters.
    A different text-to-curve consumer may use this policy for longer text. It
    must preserve the 15-character reference segment width and canonical height,
    then grow the curve span and canonical width instead of compressing extra
    anchors into the Signature's 300-unit span.
    """
    if character_count < 1:
        raise ValueError("character_count must be at least 1")

    extended = character_count > SIGNATURE_CHARACTER_LIMIT
    curve_span = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate one deterministic Signature v2.0.1 MBTI SVG."
    )
