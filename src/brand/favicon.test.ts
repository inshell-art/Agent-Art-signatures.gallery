...
describe("renderer-derived signature favicon", () => {
  it("locks the exact capital S from the signature renderer without lowercasing or redrawing", () => {
    const capture = captureSignatureComposition({ id: FAVICON_SHAPE_LOCK.id, displayText: "S", gr0kRaw: 500_000 }, developmentFixtureRenderer);
    ...
    const lowercase = captureSignatureComposition({ id: "lowercase-comparison", displayText: "s", gr0kRaw: 500_000 }, developmentFixtureRenderer);
  ...
  it("uses a square paper frame and the artwork palette unchanged in both themes", () => {
    ...
    expect(FAVICON_SVG).toContain(`color="${DEFAULT_SETTINGS.canvas.ink}"`);
    expect(FAVICON_SVG).toContain(`<rect x="160" y="141.5" width="100" height="100" fill="${DEFAULT_SETTINGS.canvas.background}"/>`);
    expect(FAVICON_SVG).toContain('fill="currentColor" stroke="none"');
    expect(FAVICON_MANIFEST.background).toBe(DEFAULT_SETTINGS.canvas.background);
    expect(FAVICON_MANIFEST.ink).toBe(DEFAULT_SETTINGS.canvas.ink);
  ...
  it.each([16, 32, 64])("renders visible ink and opaque paper margins without clipping at %dpx", async size => {
    ...
    for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
      ...
      if (x === 0 || x === size - 1 || y === 0 || y === size - 1) expect([...data.subarray(offset, offset + 3)]).toEqual([242, 234, 214]);
    }
