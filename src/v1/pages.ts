...
export function layout(options: LayoutOptions): string {
  ...
  const watermark = rehearsal
    ...
    : "";
  const footer = `<footer><div class="footer-credit">Signatures Gallery by <a class="footer-agent" href="https://x.com/AgentArt_AA" target="_blank" rel="noopener noreferrer" aria-label="Agent Art on X (opens in a new tab)">${FOOTER_X_ICON}<span>Agent Art</span><span aria-hidden="true">↗</span></a></div><a class="footer-about" href="/about"${options.footerAboutCurrent ? ' aria-current="page"' : ""}>About the work</a></footer>`;
...
export function collectionPage(params: CollectionPageParams): string {
  ...
  if ((params.localOAuthMode ?? params.fixtureMode) && params.signatures.length === 0) {
    developmentTools.push(`<section class="rehearsal-page-note"><h2>Sample preview</h2><p>This gr0k is a fixture value, not an Agent’s reading. Opening the preview creates no claim.</p><a class="grok-local-sample" href="/s/${escapeHtml(params.currentHandle)}/0.371924">Use a local sample instead →</a></section>`);
  }
