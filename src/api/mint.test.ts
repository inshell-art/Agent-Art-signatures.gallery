...
import { canonicalAssetKey, instanceAssetKey, validateMintParams } from "./mint.js";

describe("validateMintParams", () => {
  it("accepts a well-formed handle/code/postId", () => {
    const result = validateMintParams({ handle: "alice_1", code: "hfwo", postId: "123" });
  ...
  it("accepts a handle at the 15-character boundary", () => {
    ...
    expect(() => validateMintParams({ handle, code: "xxxx", postId: "1" })).not.toThrow();
  });

  it("rejects a handle over 15 characters", () => {
    const handle = "a".repeat(16);
    expect(() => validateMintParams({ handle, code: "xxxx", postId: "1" })).toThrow(ValidationError);
  });
  ...
  it("rejects a non-digit post_id", () => {
    for (const postId of ["", "abc", "12a", "-1", "1.5"]) {
      expect(() => validateMintParams({ handle: "alice", code: "xxxx", postId })).toThrow(ValidationError);
    }
  });

  it("accepts a post_id with leading zeros (it's an opaque id string, not a number)", () => {
    expect(() => validateMintParams({ handle: "alice", code: "xxxx", postId: "007" })).not.toThrow();
  });

  it("rejects an out-of-vocabulary reading code, including near-misses", () => {
    for (const code of ["hfw", "hfwoo", "HFWO", "zzzz", ""]) {
      expect(() => validateMintParams({ handle: "alice", code, postId: "1" })).toThrow(ValidationError);
    }
  });

  it("error messages name the offending value", () => {
    expect(() => validateMintParams({ handle: "bad handle", code: "xxxx", postId: "1" })).toThrow(/bad handle/);
    expect(() => validateMintParams({ handle: "alice", code: "xxxx", postId: "abc" })).toThrow(/abc/);
    expect(() => validateMintParams({ handle: "alice", code: "zzzz", postId: "1" })).toThrow(/zzzz/);
  });
});
