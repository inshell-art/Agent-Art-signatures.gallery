...
async function runtime(overrides: Partial<MintServiceAdapters> = {}) {
  ...
  const challenge = await service.createChallenge({ session, account, walletAddress: wallet.address, chainId: "31337", now });
...
describe("real local Anvil mint authorization", () => {
  ...
  it("uses real SIWE, chain time, and explicitly non-simulated transaction responses", async () => {
    ...
    const response = await app.service.issueAuthorization(app.signature, app.account, undefined, app.session);
  ...
  it("never permits seeded binding authority or fixture advancement on the local chain", async () => {
    ...
    await expect(app.service.issueAuthorization(app.signature, app.account, undefined, app.session)).rejects.toMatchObject({ code: "WALLET_NOT_LINKED" });
  });

  it("requires a fresh X identity even though local artwork provenance is development-only", async () => {
    const app = await runtime();
    ...
    await expect(app.service.issueAuthorization(app.signature, app.account, undefined, app.session)).rejects.toMatchObject({ code: "X_REAUTH_REQUIRED" });
  });

  it("reuses the same durable authorization and signature after restart", async () => {
    const app = await runtime();
    const first = await app.service.issueAuthorization(app.signature, app.account, undefined, app.session);
    ...
    expect(await service.issueAuthorization(app.signature, app.account, undefined, app.session)).toEqual(first);
  });
  ...
  it("does not invoke the signer when the prepared checkpoint fails", async () => {
    ...
    await expect(app.service.issueAuthorization(app.signature, app.account, undefined, app.session)).rejects.toThrow("disk unavailable");
  ...
  it("does not release an attestation if the issued checkpoint fails", async () => {
    ...
    await expect(app.service.issueAuthorization(app.signature, app.account, undefined, app.session)).rejects.toThrow("commit outcome unknown");
    expect(count).toBe(2);
  });
  ...
  it("durably records ambiguous signing and safely resumes the same intent after restart", async () => {
    ...
    await expect(app.service.issueAuthorization(app.signature, app.account, undefined, app.session)).rejects.toMatchObject({ code: "SIGNER_UNAVAILABLE" });
    ...
    const resumed = await service.issueAuthorization(app.signature, app.account, undefined, app.session);
  ...
  it("checks local expiration against chain time, without releasing the replay lock", async () => {
    ...
    await expect(app.service.issueAuthorization(app.signature, app.account, undefined, app.session)).rejects.toMatchObject({ code: "AUTHORIZATION_EXPIRED" });
    expect(() => app.service.revokeBinding(app.account.xUserId, now)).toThrowError(expect.objectContaining({ code: "LIVE_AUTHORIZATION_EXISTS" }));
  });
});
